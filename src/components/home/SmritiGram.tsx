...
{isApplicationFormOpen && (
  <div
    className="fixed inset-0 z-[9998] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm sm:p-6"
    role="dialog"
    aria-modal="true"
    aria-label="Smriti Gram admission form"
    onClick={() => setIsApplicationFormOpen(false)}
  >
    <div
      className="relative max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-[1.75rem] bg-white shadow-2xl sm:rounded-[2rem]"
      onClick={(event) => event.stopPropagation()}
    >
      <div className="sticky top-0 z-10 flex items-center justify-between border-b border-[#eadfd2] bg-white px-5 py-4 sm:px-7">
        <div>
          <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[#ED6439]">
            Admission For Residential Care
          </p>
          <h3 className="mt-1 font-display text-2xl font-bold text-[#263746] sm:text-3xl">
            Click here to fill our form
          </h3>
        </div>
        ...
        <button
          ...
          className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-[#FFF1DC] text-[#263746] transition-colors hover:bg-[#ED6439] hover:text-white"
        >
          <X className="h-5 w-5" />
        </button>
      </div>
      ...
      <form
        className="space-y-7 p-5 sm:p-7 md:p-9"
        onSubmit={(event) => {
          event.preventDefault();
          const formData = new FormData(event.currentTarget);
          const subject = encodeURIComponent("Smriti Gram Admission Application");
          const body = encodeURIComponent(
            Array.from(formData.entries())
              .map(([key, value]) => `${key}: ${value}`)
              .join("\\n")
          );
          window.location.href = `mailto:contact@nightingaleseldercare.com?subject=${subject}&body=${body}`;
        }}
      >
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
          <FormField label="Name of the person" name="Name of the person" required />
          <FormField label="Age" name="Age" type="number" required />
          <FormField label="Contact number" name="Contact number" type="tel" required />
          <FormField label="Family income" name="Family income" required />
        </div>

        <div>
          <p className="mb-4 font-display text-xl font-bold text-[#263746]">
            Admission Criteria
          </p>

          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <label className="rounded-2xl border border-[#eadfd2] bg-[#FFF8EF] p-4">
              <span className="text-sm font-bold text-[#263746]">Confirmed dementia diagnosis</span>
              <select name="Confirmed dementia diagnosis" required className="mt-3 w-full rounded-xl border border-[#eadfd2] bg-white px-3 py-2.5 text-sm text-[#526574] outline-none focus:border-[#ED6439]">
            ...
            <label className="rounded-2xl border border-[#eadfd2] bg-[#FFF8EF] p-4">
              <span className="text-sm font-bold text-[#263746]">Need for 24-hour care</span>
              <select name="Need for 24-hour care" required className="mt-3 w-full rounded-xl border border-[#eadfd2] bg-white px-3 py-2.5 text-sm text-[#526574] outline-none focus:border-[#ED6439]">
                <option value="">Select</option>
                <option value="Yes">Yes</option>
                <option value="No">No</option>
              </select>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {[
              ...
              <label key={item} className="flex items-start gap-3 rounded-2xl border border-[#eadfd2] bg-white p-4 text-sm leading-6 text-[#526574]">
                <input type="checkbox" name="Preference criteria" value={item} className="mt-1 h-4 w-4 shrink-0 accent-[#ED6439]" />
                <span>{item}</span>
              </label>
            ))}
          </div>
        ...
        <label className="block">
          <span className="text-sm font-bold text-[#263746]">Stage and medical condition</span>
          <textarea
            name="Stage and medical condition"
            rows={4}
            className="mt-2 w-full rounded-2xl border border-[#eadfd2] bg-[#FFF8EF] px-4 py-3 text-sm leading-6 text-[#526574] outline-none transition-colors focus:border-[#ED6439]"
          />
        </label>

        <label className="block">
          <span className="text-sm font-bold text-[#263746]">Medical records and dementia diagnosis</span>
          <input
            type="text"
            name="Medical records and dementia diagnosis"
            placeholder="Mention the available medical records"
            className="mt-2 w-full rounded-2xl border border-[#eadfd2] bg-[#FFF8EF] px-4 py-3 text-sm text-[#526574] outline-none transition-colors focus:border-[#ED6439]"
          />
        </label>

        <div className="flex flex-col gap-3 border-t border-[#eadfd2] pt-6 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-xs leading-5 text-[#526574]">
            Please submit the required documents along with the application.
          </p>
          <button
            type="submit"
            className="inline-flex items-center justify-center gap-2 rounded-full bg-[#ED6439] px-6 py-3.5 text-sm font-bold text-white transition-transform hover:-translate-y-0.5"
          >
            Submit Application
            <ArrowRight className="h-4 w-4" />
          </button>
...
function FormField({
  ...
  return (
    <label className="block">
      <span className="text-sm font-bold text-[#263746]">{label}</span>
      <input
        type={type}
        name={name}
        required={required}
